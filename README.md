...
- **データ駆動な活用システム**: 活用ルール（どの形にどの接尾辞を足すか）を `rules/common.yaml` で定義。
- **統一されたルール管理**: 動詞、形容詞、定型句（フレーズ）を `rules/custom_rules.yaml` で一括管理。
- **部分的な活用生成**: `allowed_kinds` を指定することで、必要な活用形のみを生成し辞書の肥大化を抑制。
- **特殊活用への対応**: 「ある→あらず」「行く→行った（促音便）」などの例外処理を内包。
...
- `src/conjugators/`
    - `verb.py`, `adjective.py`, `phrase.py`: それぞれの品詞・形式に応じた活用生成ロジック。
